...
def _safe(s):
    """fpdf2's core Helvetica is latin-1; replace unmappable chars."""
    return str(s).encode("latin-1", errors="replace").decode("latin-1")


def generate_report(df, feie, ftc):
    pdf = TaxReportPDF()
    ...
    pdf.cell(0, 8, "Generated: " + datetime.datetime.now().strftime("%B %d, %Y"), ln=True, align="C")
    pdf.cell(0, 8, "Transactions analyzed: " + str(len(df)), ln=True, align="C")
    pdf.ln(60)
